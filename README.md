...
KIL, the KiCad Intent Language, compiles circuit definitions and PCB layout intent from `*.kil.json` files into a KiCad 10 project. Edit the JSON source and regenerate the KiCad files when it changes. Generated files are disposable output.

The format is experimental. It supports reusable modules, multiple schematic sheets, and boards with 2 to 32 copper layers.

## Installation

KIL requires KiCad 10 and its symbol and footprint libraries.
...
To install from a source checkout, use Rust 1.90 or newer:
...
From a checkout of this repository, inspect an example, lock its library dependencies, then check and build it:
...
Query a library symbol or print the editor schemas:
...
KIL keeps the source document separate from the resolved compiler model. During compilation, it expands module instances, assigns printed references, and resolves terminals and library assets without modifying the source. Stable source identities determine the generated KiCad object identities.
...
An instance uses the module's supplied schematic or PCB view when it declares the corresponding `schematic` or `pcb` transform. Omit either to supply that view from the parent, using qualified part IDs. Electrical hierarchy does not require KiCad sheet hierarchy.
...
Routing fingerprints include resolved library contents, per-component footprint IDs, nets, PCB geometry, rules, and routing policy. Schematic placement, values, and printed reference changes do not invalidate copper. Caches written before this fingerprint recipe become stale (`ROUTE006`) under `kil check` and `kil build`; run `kil route FILE` once after upgrading to regenerate them.

`rules.minimum_track_width` is a requirement. `rules.preferred_track_width` is the routing default. Named `rules.net_classes` declare explicit net membership, clearance, minimum and preferred widths, and allowed copper layers. Class requirements must meet board-wide minimums; multiple class assignments are errors. The compiler checks geometry and emits native KiCad net classes and a `.kicad_dru` file with custom width/layer rules. The custom rules file is part of the published project. See KiCad's [custom rule documentation](https://docs.kicad.org/10.0/en/pcbnew/pcbnew.html#custom-design-rules).
...
Generated child files live in `<project>.sheets/`, owned by the compiler and replaced with the other generated artifacts on every build. Sheet filenames are stable generated identifiers, not source paths. See [multi-sheet-divider](examples/multi-sheet-divider.kil.json).
